...
(defui Level2Form
  ...
  static f/IForm
  (fields [this] [(f/subform :leaf1 :one)
                  (f/subform :leaf2 :one)
                  (f/subform :leaf3 :one)]))

(defui OtherRootForm
  static uc/InitialAppState
  (initial-state [this {:keys [id]}] {:id id :other true})
  static om/IQuery
  (query [this] [:id :other])
  static om/Ident
...
(defui Level3Form
  ...
  static f/IForm
  (fields [this] [(f/subform :level2 :one)]))
...
(specification "get-forms"
  (assertions
    "Obtains a list of nested forms and idents that are both declared as subforms and are present."
...
(specification "update-forms"
  (let [new-state (f/update-forms Level3Form nested-form-db [:level3 1] (fn [f] (assoc f :touched true)))]
    (assertions
