...
(comment
  ...
  (defui ^:once PersonForm
    ...
    (initial-state [this params] (f/build-form this (or params {})))
    static f/IForm
    (subforms [this] [(f/subform-element :person/phone-numbers :many)])
    (fields [this] [(f/id-field :db/id)
    ...
    (render [this]
      (let [{:keys [person/phone-numbers] :as props} (om/props this)
        ...
        (dom/div #js {:className "form-horizontal"}
          ...
          (f/form-field form :display-string)
          (dom/div nil
...
(comment
  (defui I18NString
    static f/IForm
    (form-elements [this] [(f/text-input :value 'not-empty)])
    static om/IQuery
    (query [this] [:db/id])
    static om/Ident
    (ident [this props] [:strings/by-id (:db/id props)])
    Object
    (render [this]
      (dom/div nil
        (str locale ":")
        (f/form-field (om/props this) :value))))

  (defui ^:once MyI18NThing
    static uc/InitialAppState
    (initial-state [this params] (f/build-form MyI18NThing
                                               {:db/id         33
                                                :display-title [{:db/id 1 :langauge/locale :en :value "Hello"}
                                                                {:db/id 2 :langauge/locale :es :value "Hola"}
                                                                {:db/id 3 :langauge/locale :ja :value "kanban wa"}]}))
    static f/IForm
    (form-elements [this] [(f/subform-element :something)
                           (f/form-switcher-input :display-title I18NString :language/locale)
                           (f/form-switcher-input :summary I18NString :language/locale)
                           (f/form-switcher-input :description I18NString :language/locale)])
    static om/IQuery
    (query [this] [:db/id {:display-title (om/get-query I18NString)} :ui/form :ui/my-selected-locale])
    static om/Ident
    (ident [this props] [:phone/by-id (:db/id props)])
    Object
    (render [this]
      (let [{:keys [ui/my-selected-locale] :as form} (om/props this)]
        (dom/div #js {:className "form-horizontal"}
          (f/form-field form :display-title :key my-selected-locale)
          (f/form-field form :summary :key my-selected-locale)
          (f/form-field form :description :key my-selected-locale)

          (f/invalid? form :display-title)
          )))))
